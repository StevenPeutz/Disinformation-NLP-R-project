...
stm_model <- stm(text_dfm, K = 5, init.type = "Spectral")
```
...
```{r}
lda_model <- LDA(text_dtm, k = 5, control = list(seed = 10)) 
...
With our STM 5 topic model based on the 1000 text sample we have the followinf 5 topics:  
1 'Western Politcs' (keywords: west(ern), eu,  europ(ean), polit(ics/icians), belarus.  
2 'Nato vs Russia' (keywords: nato, russia(n), war, europ(e).  
3 'Ukraine' (keywords: Ukrain(ean), and to a lesser extent kyiv, people, president, donbas, poroshenko, coup, conflict)  
4 'Russia' (keywords: russia, and to a lesser extent crimea, military, sanction)  
5 'Chemical attack Syria' (keywords: syria, atack, chemic(al), terrorist, prepar(e)  


With our LDA 5 topic model based on the 1000 text sample we have the followinf 5 topics:  
1 'Chemical attack Syria' (keywords: ukraine, syria, atack, chemic(al), terrorist, prepar(e)
2 'Crimea' (keywords: russia, crimea, ukraine and to a lesser extent military, president, turkey, british, putin)  
3 'The Donbas region' (keywords: ukraine, russia, donbas, elestions, kyiv)
4 'EU and Nato (vs Russia)' (keywords: europ(ean), eu, nato, russia(n), war)  
5 'Belarus and Russia vs the west and nato' (keywords: belarus, west(ern) nato, war)

 

Sampling
```{r}
 df_manual <- sample_n(df2, 10)
 df_manual
```
...
SOme notes for us:  
1) I still see some nbsp's in the data ()  
2) US, we should replace us with 'usa', because it think it is being deleted (after small caps after all it is same as 'us'..)  
3) Results of the topic modelling (also lda) still change quite strongly even with 1000 sample size..  
4) is " str_replace_all("european union", "eu") " an option? (of course prior to replacing european, but are two word combincations possible like this?)  
5) as eduardo ssuggested a while back, should we try also with 'russia' taken out?  
