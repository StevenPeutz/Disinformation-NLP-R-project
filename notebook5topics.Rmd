...
df3$stem <- df3$stem %>%   
  ...
  str_replace_all("european", "europ") #since 'europe' already is stemmed to 'europ', but 'european' was not. 'eu' remains a separate term due its 'union' context.
...
```
Here the CaoJuan2009 metric suggest a cutoff at 4 topics, with 6 as second best. The Deveaud2014 metric is more linear here and has a less clear cutoff, mostly suggesting that 5 however is considerable worse than 4, but also that 4 topics is worse than 3. Combining the two, and the relative clarity of the curve in the CaoJuan2009 metric we will set our STM and LDA models to 4 topics. 
